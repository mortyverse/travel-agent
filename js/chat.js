...
async function handleSend() {
  ...
  try {
    ...
    const systemPrompt = planningDone ? MODIFICATION_SYSTEM_PROMPT : COLLECTION_SYSTEM_PROMPT;
    const reply = await callGemini(
      conversationHistory,
      systemPrompt,
      planningDone ? {} : { maxOutputTokens: 512, temperature: 0.5 }
    );
    removeTyping(typingEl);
    conversationHistory.push({ role: 'model', parts: [{ text: reply }] });

    if (!planningDone && reply.includes('PLANNING_START')) {
...
const MODIFICATION_SYSTEM_PROMPT = `당신은 여행 일정 수정 전문가입니다.
이전에 생성된 여행 일정표와 대화 내역을 참고하여, 사용자의 수정 요청을 반영한 답변을 한국어로 작성하세요.
일정표 전체를 수정하는 경우 기존 형식을 유지하고 수정된 부분을 반영하세요.
마지막에 "추가로 수정하고 싶은 부분이 있으신가요?" 로 마무리하세요.`;

async function callGemini(history, systemPrompt, genConfig = {}) {
